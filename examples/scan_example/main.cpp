...
using namespace bglib;

volatile int tty_fd = -1;

class BleScanner: public bglib::Ble
{
  struct gap_ad_t {
    uint8 len;
    ...
    while (p < end_p)
    {
      switch (ad->ad_type)
      {
        ...
        case gap_ad_type_localname_complete:
          strncpy(str, (char*)ad->data, ad->len - 1);
          printf("name: \"%s\" ", str);
          break;
  ...
  void evtGapScanResponse(const struct ble_msg_gap_scan_response_evt_t* msg)
  {
  ...
  void rawWrite(uint8 len1, const uint8* data1, uint16 len2, const uint8* data2)
  {
...
public:
  ...
  BleScanner()
  {
    setMsgHandler<ble_msg_gap_scan_response_evt_t>(std::bind(&BleScanner::evtGapScanResponse, this, std::placeholders::_1));
  }

  int readMessage(int timeout_ms)
  {
    ...
    const struct ble_msg *msg = this->getMsgHdr(hdr);
...
int main(int argc, char** argv)
{
  ...
  s.cmdGapEndProcedure();
  s.cmdGapDiscover(gap_discover_observation);

  while (1)
  {
    if (s.readMessage(UART_TIMEOUT) > 0)
      break;
  }
